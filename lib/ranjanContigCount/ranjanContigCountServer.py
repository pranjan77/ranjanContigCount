...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'ranjanContigCount'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from ranjanContigCount.ranjanContigCountImpl import ranjanContigCount
impl_ranjanContigCount = ranjanContigCount(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['ranjanContigCount.count_contigs_async'] = ['ranjanContigCount', 'count_contigs']
async_check_methods['ranjanContigCount.count_contigs_check'] = ['ranjanContigCount', 'count_contigs']
sync_methods['ranjanContigCount.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'ranjanContigCount'
        ...
        self.rpc_service.add(impl_ranjanContigCount.count_contigs,
                             name='ranjanContigCount.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['ranjanContigCount.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "ranjanContigCount but no authentication header was passed"
                            raise err
