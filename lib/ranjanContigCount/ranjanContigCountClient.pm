package ranjanContigCount::ranjanContigCountClient;
...
ranjanContigCount::ranjanContigCountClient
...
sub new
{
    ...
    my $self = {
	client => ranjanContigCount::ranjanContigCountClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	method => "ranjanContigCount.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "ranjanContigCount.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for ranjanContigCount::ranjanContigCountClient\n";
    }
    if ($sMajor == 0) {
        warn "ranjanContigCount::ranjanContigCountClient version is $svr_version. API subject to change.\n";
    }
}
...
package ranjanContigCount::ranjanContigCountClient::RpcClient;
